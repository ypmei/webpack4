import '../styles/components/tabnav.css'
...
export default class TabNav extends Component {
  static Panel = class Panel extends Component {
    ...
    render(){
      return (
        <div className={cx('widget-content', 'bg-white', this.props.flex)}>
          {this.props.children}
        </div>
      )
    }
          ...
          children.map((panel, index) => {
            ...
            return panel.props.href ? (
              <Link
                ...
                {
                  panel.props.tip ? (
                    <span className="iconfont tip-icon" dangerouslySetInnerHTML={{__html: '&#xe7d6;'}} data-tip={panel.props.tip}></span>
                  ) : null
                }
              </Link>
            ) : (
